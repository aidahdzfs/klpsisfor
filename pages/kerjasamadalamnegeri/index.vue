    ...
    <main class="overflow-x-hidden">
        ...
        <section class="pt-24">
            <div class="h-[202px] relative">
                ...
                <div class="w-[474px] h-[45px] relative top-20 left-20">
                    <a href="#" class="w-[302px] h-[45px] left-[172px] absolute text-slate-300 text-2xl font-semibold">Kerja Sama Dalam Negeri</a>
        ...
        <section class="mt-10 py-8">
            <div id="pengembangankarakter">
                <div v-for="item in dalamnegeriData" :key="item.id" class="pb-20">
                    ...
                    <img src="../assets/image/kerjasamadn.png" class="flex items-center justify-center px-52"/>
                </div>
            </div>
        </section>
    </main>
...
<script setup>
    ...
    const endpoint = "http://localhost:8055/items/kerjasamadn"
    const dalamnegeriData = ref('');

    async function getDalamnegeri() {
        const api = await fetch(endpoint)
        const data = await api.json()
        console.log(data.data)
        dalamnegeriData.value = data.data
    }
