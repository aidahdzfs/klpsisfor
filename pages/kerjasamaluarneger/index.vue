    ...
    <main class="overflow-x-hidden">
        ...
        <section class="pt-24">
            <div class="h-[202px] relative">
                ...
                <div class="w-[474px] h-[45px] relative top-20 left-20">
                    <a href="#" class="w-[302px] h-[45px] left-[172px] absolute text-slate-300 text-2xl font-semibold">Kerja Sama Luar Negeri</a>
        ...
        <section class="mt-10 py-8">
            <div id="pengembangankarakter">
                <div v-for="item in luarnegeriData" :key="item.id" class="pb-20">
                    ...
                    <p class="px-28 font-medium pb-10" v-html="item.isi"></p>
                    <h1 class=" text-center font-bold pb-5">DAFTAR KERJASAMA UNHAS, FAKULTAS TEKNIK DENGAN UNIVERSITAS LUAR NEGERI</h1>
                    <img src="../assets/image/kerjasamaln.png" class="flex items-center justify-center px-52"/>
                </div>
            </div>
        </section>
    </main>
...
<script setup>
    ...
    const endpoint = "http://localhost:8055/items/kerjasamaln"
    const luarnegeriData = ref('');

    async function getLuarnegeri() {
        const api = await fetch(endpoint)
        const data = await api.json()
        console.log(data.data)
        luarnegeriData.value = data.data
    }
