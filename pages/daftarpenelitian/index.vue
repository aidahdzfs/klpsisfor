<template>
    <main class="overflow-x-hidden">
        ...
        <section class="pt-24">
            <div class="h-[202px] relative">
                ...
                <div class="w-[474px] h-[45px] relative top-20 left-20">
                    <a href="#" class="w-[302px] h-[45px] left-[172px] absolute text-slate-300 text-2xl font-semibold">Daftar Penelitian LBE</a>
        ...
        <section class="mt-10 py-8">
            <div id="daftarpenelitian">
                <div v-for="item in penelitianData" :key="item.id" class="pb-20">
...
<script setup>
    ...
    const endpoint = "http://localhost:8055/items/daftarpenelitian"
    const penelitianData = ref('');

    async function getPenelitian() {
        const api = await fetch(endpoint)
        const data = await api.json()
        console.log(data.data)
        penelitianData.value = data.data
    }
